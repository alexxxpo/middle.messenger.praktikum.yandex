...
export default class Profile extends Block<ProfileType> {
  ...
  init(): void {
    ...
    const oldPasword = new PField({
      label: 'Старый пароль', type: 'password', value: 'sdafadfa', name: 'old_password', events: {
        blur: [e => onChangeInput(e, this.children.oldPasword, 'Некорректный формат', ...conditions.password)]
      }
    })
    const newPassword = new PField({
      label: 'Новый пароль', type: 'password', value: 'ivanivanov', name: 'new_password', events: {
    ...
    const newPasswordAgain = new PField({
      label: 'Повторите новый пароль', type: 'password', value: 'ivanivanov', name: 'new_password', events: {
    ...
    const popup = new Popup({
      ...
      clickButton: e => this.setProps({ popup: false })
    })
  ...
  onChangeData(): void {
    Object.values(this.children).forEach(child => {
      if ('name' in child && child.name === 'email' || 'login' || 'first_name' || 'second_name' || 'display_name' || 'phone') {
        child.setProps({
          disabled: false,
        })
      }
  ...
  onChangePassword(): void {
    Object.values(this.children).forEach(child => {
      if ('name' in child && child.name === 'old_password' || 'new_password' || 'new_password_again') {
        child.setProps({
          disabled: false,
        })
      }
  ...
  onSaveData(): void {
    Object.values(this.children).forEach(child => {
      if ('name' in child && child.name === 'email' || 'login' || 'first_name' || 'second_name' || 'display_name' || 'phone' || 'old_password' || 'new_password' || 'new_password_again') {
        child.setProps({
          disabled: true,
        })
      }
    ...
    return `
        <main class="page profilePage">
            <div class="profilePage__userInfo">
                ...
                <form>
                    <h2 class="profilePage__title">${this.children.displayName.props.value}</h2>   
